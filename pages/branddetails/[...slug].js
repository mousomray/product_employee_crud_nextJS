...
import { singlebrand, deleteproduct } from '../function_folder/allfunction';
...
const Index = () => {
    ...
    const getData = async () => {
        const response = await singlebrand(slug);
        console.log("Single brand response...", response);
        return response;
    };

    const { isLoading, isError, data: singlebranddata, refetch } = useQuery({
        queryKey: ["singlebranddata", slug],
        queryFn: getData,
    });
    ...
    const filteredProduct = Array.isArray(singlebranddata) && singlebranddata?.filter((product) =>
